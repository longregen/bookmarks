...
export async function runDetailPanelTests(adapter: TestAdapter, runner: TestRunner): Promise<void> {
  ...
  await runner.runTest('Detail panel backdrop click closes panel', async () => {
    ...
    // Click backdrop using JavaScript to avoid obstruction issues
    // The backdrop may be partially covered by the detail panel at the center
    await page.evaluate(`
      (() => {
        const backdrop = document.getElementById('detailBackdrop');
        if (backdrop) {
          // Dispatch click event directly to bypass obstruction checks
  ...
  await runner.runTest('Delete button with cancellation does not delete bookmark', async () => {
    ...
    // Click delete button using JavaScript to ensure visibility
    await page.evaluate(`
      (() => {
