...
export function createBookmarkRoutes(deps: AppDependencies): Hono<{ Variables: AppVariables }> {
  ...
  bookmarks.post('/', async (c) => {
    ...
    if (existing) {
      await deps.db.prepare('UPDATE bookmarks SET title = ?, html = ?, status = \'pending\', updated_at = ? WHERE id = ?').bind(body.title, body.html, timestamp, existing.id).run();
    ...
    await deps.db.prepare('INSERT INTO bookmarks (id, user_id, url, title, html, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, \'pending\', ?, ?)').bind(id, auth.userId, body.url, body.title, body.html, timestamp, timestamp).run();
